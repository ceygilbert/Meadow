...
const PCBuilder: React.FC = () => {
      ...
      try {
        ...
        if (prodRes.data && subcatRes.data && brandRes.data) {
          const mapProducts = (
            ...
            const subcatIds = subcatRes.data
              .filter((c) =>
                categoryKeywords.some((key) =>
                  c.name.toLowerCase().includes(key),
                ),
              )
              .map((c) => c.id);

            const brandIds = brandKeywords
              ? brandRes.data
                  .filter((b) =>
                    brandKeywords.some((key) =>
            ...
            return prodRes.data
              .filter((p: any) => {
                const matchesCat = subcatIds.includes(p.subcategory_id);
                const matchesBrand = brandIds
          ...
          setCpuData({
            ...
            items: mapProducts(["processor", "cpu"], ["intel", "amd"]),
          });
          setCoolerData({
            filters: ["ALL", "AIR", "LIQUID"],
            items: mapProducts(["cooler", "cooling"]),
          });
          setCaseData({
            filters: ["ALL", "MID TOWER", "FULL TOWER", "ITX"],
            items: mapProducts(["case", "chassis"]),
          });
          setMbData({
            filters: ["ALL", "Z790", "B760", "X670", "B650"],
            items: mapProducts(["motherboard", "mainboard"]),
          });
          setGpuData({
            filters: ["ALL", "NVIDIA", "AMD"],
            items: mapProducts(["graphic", "gpu"], ["nvidia", "amd"]),
          });
          setRamData({
            filters: ["ALL", "DDR5", "DDR4"],
            items: mapProducts(["ram", "memory"]),
          });
        }
      } catch (err) {
  ...
  const COMPONENT_DATA: Record<
    string,
    { filters: string[]; items: BuildItem[] }
  > = {
    promotion: {
  ...
  const removeItem = (catId: string, index?: number) => {
    setSelections((prev) => {
      ...
      if (Array.isArray(newSels[catId]) && index !== undefined) {
        newSels[catId] = newSels[catId].filter(
  ...
  const diagnostics = useMemo(() => {
    ...
    const procPower =
      cpuWeight * 0.7 +
      getWeight("motherboard") * 0.15 +
      getWeight("cpu cooler") * 0.15;
  ...
  const radarData = useMemo(() => {
    ...
    // Speed: CPU (50%) + RAM (50%)
    const speed =
      Math.round(getWeight("cpu") * 0.5 + getWeight("ram") * 0.5) || 0;

    // Storage: Storage (SSD)
    const storageWeight =
      Array.isArray(selections["storage"]) && selections["storage"].length > 0
        ? selections["storage"].reduce(
            (acc: number, curr: any) => acc + (curr.perfWeight || 0),
            0,
          ) / selections["storage"].length
        : 0;
    const storage = Math.round(storageWeight) || 0;

    // Cooling: CPU Cooler (70%) + Fans (30%)
    const cooling =
      Math.round(getWeight("cpu cooler") * 0.7 + getWeight("fans") * 0.3) || 0;

    // Stability: PSU (60%) + Motherboard (40%)
    const stability =
      Math.round(getWeight("psu") * 0.6 + getWeight("motherboard") * 0.4) || 0;
    ...
    <div className="min-h-screen bg-[#050607] font-sans text-slate-100 selection:bg-rose-600 selection:text-white overflow-x-hidden">
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-start">
            ...
            {schematicZones.map((zone, zIdx) => (
              <div key={zIdx} className="space-y-6">
                <div className="flex items-center gap-4 px-4">
                  ...
                  <h3 className="font-serif text-2xl font-light tracking-wide text-white/60">
                    {zone.group}
                  </h3>
                </div>
                    ...
                    return (
                      <div key={item.id} className="space-y-4">
                        {isMulti &&
                          ...
                          selected.map((sItem: any, sIdx: number) => (
                            <div
                              key={sIdx}
                              className="relative p-6 bg-[#0c0d0e] border border-rose-600 rounded-[3rem] animate-in fade-in slide-in-from-top-4 duration-500 group overflow-hidden"
                            >
                              <div className="flex items-center gap-8 relative z-10">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-[14px] font-black text-rose-500 uppercase tracking-widest">
                                    RM {sItem.price.toLocaleString()}
                                  </p>
                                </div>

                                <div className="flex items-center gap-4">
                                  <button
                                    onClick={() => removeItem(item.id, sIdx)}
                                    className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:bg-rose-600 transition-all"
                                  >
                                    <Trash2 size={20} />
                                  </button>
                                </div>
                              ...
                              >
                                <div className="flex items-center justify-between relative z-10">
                                  <div className="flex items-center gap-10">
                                    <div
                                      className={`w-20 h-20 rounded-[1.5rem] flex items-center justify-center transition-all duration-700 ${hoveredZone === item.id ? "bg-rose-600 text-white shadow-[0_0_40px_rgba(225,29,72,0.5)]" : "bg-white/10 text-white/50"}`}
                                    >
                                      {item.icon}
                                    </div>
                                    <div>
                                      <p className="text-[15px] font-black uppercase tracking-[0.1em] text-white/60 group-hover:text-white transition-colors">
                                        {item.label}
                                      </p>
                                      <p className="text-[11px] font-bold text-rose-500/80 mt-2 uppercase tracking-[0.2em]">
                                        Pending Integration
                                      </p>
                                    </div>
                                  </div>
                                  <div
                                    className={`w-12 h-12 rounded-full border border-white/20 flex items-center justify-center transition-all ${hoveredZone === item.id ? "bg-white text-black scale-110 shadow-2xl" : "text-white/30"}`}
                                  >
                                    <Plus size={24} />
                                  </div>
                                </div>
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-32 z-50">
              <div
                className={`bg-[#0A0B0C]/80 backdrop-blur-xl border transition-all duration-500 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden group relative ${radarPulse ? "border-rose-600 shadow-[0_0_40px_rgba(225,29,72,0.3)] scale-[1.02]" : "border-white/10"}`}
              >
                ...
                <div className="relative z-10 p-8 border-b border-white/5">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-[8px] font-black text-rose-500 uppercase tracking-[0.3em] mt-1">
                        Ability Matrix
                      </p>
                    </div>
                ...
                <div className="relative z-10 p-8 space-y-6">
                  <div className="space-y-4">
                    {[
                      {
                        id: "procPower",
                        label: "Processing Power (CPU)",
                        value: `${diagnostics.procPower}%`,
                        color: "bg-rose-600",
                        width: diagnostics.procPower,
                      },
                      {
                        id: "graphPower",
                        label: "Graphical Performance (GPU)",
                        value: `${diagnostics.graphPower}%`,
                        color: "bg-rose-700",
                        width: diagnostics.graphPower,
                      },
                      {
                        id: "memAlloc",
                        label: "Memory (RAM Size/Speed)",
                        value: `${diagnostics.memAlloc}%`,
                        color: "bg-rose-800",
                        width: diagnostics.memAlloc,
                      },
                      {
                        id: "stability",
                        label: "Storage",
                        value: diagnostics.stability,
                        color: "bg-white",
                        width: diagnostics.stability !== "None" ? 100 : 0,
                      },
                    ].map((metric) => {
                      const isHoverLinked =
                        hoveredZone &&
                        metricMapping[hoveredZone]?.includes(metric.id);

                      return (
                        <div
                          key={metric.id}
                          className={`space-y-2 transition-all duration-500 ${isHoverLinked ? "scale-105" : ""}`}
                        >
                          <div className="flex items-center justify-between px-1">
                            <span
                              className={`text-[9px] font-black uppercase tracking-[0.2em] transition-colors ${isHoverLinked ? "text-rose-500" : "text-white/60"}`}
                            >
                              {metric.label}
                            </span>
                          ...
                          <div
                            className={`h-[2px] w-full bg-white/10 rounded-full overflow-hidden relative ${isHoverLinked ? "shadow-[0_0_20px_rgba(225,29,72,0.4)]" : ""}`}
                          >
                            <div
        ...
        <div className="max-w-[1600px] mx-auto bg-[#0A0B0C]/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-2 flex items-center justify-between shadow-[0_40px_120px_rgba(0,0,0,1)] pointer-events-auto overflow-hidden">
          ...
          <div className="flex items-center gap-12 px-8">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-rose-500 uppercase tracking-[0.6em]">
                Total Price
              </span>
              <span className="font-serif text-xl font-light text-white tracking-tight italic mt-1">
                RM{" "}
            ...
            <div className="hidden md:flex flex-col items-start border-l border-white/10 pl-12">
              ...
              <div className="flex gap-2 mt-3">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className={`w-8 h-[1.5px] rounded-full transition-all duration-700 ${i < selectedCount ? "bg-rose-600 shadow-[0_0_10px_rose-600]" : "bg-white/10"}`}
                  ></div>
                ))}
              </div>
            </div>
      ...
      {activePopup && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div
            className="absolute inset-0 bg-[#050607]/80 backdrop-blur-sm"
          ...
          <div className="relative w-full max-w-[650px] bg-[#0A0B0C] border border-white/10 rounded-[2rem] shadow-[0_30px_100px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col max-h-[80vh]">
            <div className="px-6 py-4 border-b border-white/10 shrink-0 bg-gradient-to-b from-white/[0.03] to-transparent relative">
              <button
                onClick={() => setActivePopup(null)}
                className="absolute top-1/2 -translate-y-1/2 right-6 w-8 h-8 rounded-full bg-white/10 text-white/50 hover:text-white hover:bg-rose-600 transition-all z-20 flex items-center justify-center shadow-md"
              >
                <X size={16} />
              </button>
              ...
              <div className="max-w-[450px] relative group">
                <Search
                  className="absolute left-5 top-1/2 -translate-y-1/2 text-white/40 group-focus-within:text-rose-500 transition-colors"
                  size={16}
                />
                <input
                  type="text"
              ...
              {currentPopupData.filters.map((filter) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all border ${
                    activeFilter === filter
                      ? "bg-rose-600/20 text-rose-500 border-rose-600/50"
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2.5 scrollbar-hide">
              {filteredItems.map((item, idx) => (
                <div
                  ...
                  className="group px-4 py-3 rounded-2xl border border-white/5 bg-white/[0.02] hover:border-rose-600/40 hover:bg-white/[0.06] flex items-center gap-4 transition-all duration-300 cursor-pointer"
                >
                  <div
                    className="w-16 h-16 bg-slate-900 rounded-xl overflow-hidden shrink-0 border border-white/10 group-hover:border-rose-600/50 transition-colors relative z-10"
                    onClick={(e) => {
                      e.stopPropagation();
                      e.preventDefault();
                      const fallbackImg = "https://images.unsplash.com/photo-1587831990711-23ca6441447b?q=80&w=1000&auto=format&fit=crop";
                      setEnlargedImage(item.image_url || fallbackImg);
                    }}
                  >
                    <img
                      src={item.image_url || "https://images.unsplash.com/photo-1587831990711-23ca6441447b?q=80&w=1000&auto=format&fit=crop"}
                      alt={item.name}
                      className={`w-full h-full object-contain ${!item.image_url ? 'opacity-40 grayscale' : ''}`}
                    />
                  </div>
            ...
            <div className="px-6 py-3 bg-white/[0.02] border-t border-white/10 flex items-center justify-between shrink-0">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-white/5 text-white/50 border border-white/5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] hover:bg-rose-600 hover:text-white hover:border-rose-600 transition-all shadow-xl"
              >
                Flush
              </button>
            </div>
      ...
      {enlargedImage && (
        <div
          className="fixed inset-0 z-[2000] flex items-center justify-center p-8 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300"
        ...
        >
          ...
          <div
            className="relative w-full max-w-5xl aspect-square md:aspect-video flex items-center justify-center p-8"
